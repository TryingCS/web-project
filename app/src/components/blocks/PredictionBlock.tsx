...
import type { PredictionBlockContent } from '@/types';
...
interface PredictionBlockProps {
  content: PredictionBlockContent;
}

export function PredictionBlock({ content }: PredictionBlockProps) {
    ...
    <Card className="border-l-4 border-l-amber-400 shadow-md">
      <CardContent className="p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-gray-900">Prediction Question</h3>
        </div>
        ...
        >
          {content.options.map((option, index) => (
            <div key={index}>
              <RadioGroupItem value={index.toString()} id={`pred-${index}`} disabled={isSubmitted} className="sr-only" />
              <Label
                htmlFor={`pred-${index}`}
